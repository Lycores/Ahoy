...
overflow不会继承，如果在外面有一个

clamp的特点，它会从小到大选择合适的尺寸

## 最大的困难：

##### 结论：最大的困难就是在flex布局下的智能size，智能typography的同时保证不出现任何overflow的同时让页面看起来好看。
...
##### 阶段1: 问题的解答：如果不显性表达，flex item是没有自己的width的。而在响应式里面显性表达宽度意味着要一直使用JS进行计算并且重复渲染。flex item的width完全由它孩子的宽度决定，然而在视频里，clamp是一个智能的css表达式，我们无法精确控制它的宽度，所以它孩子的宽度撑起父亲的宽度，当我们继续减少window width,自然会出现content overflow
